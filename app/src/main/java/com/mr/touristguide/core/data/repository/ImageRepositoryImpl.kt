...
class ImageRepositoryImpl @Inject constructor(
    ...
    override suspend fun searchImages(term: String, pageSize: Int, maxImages: Int): Flow<PagingData<UnsplashImage>> {
        return Pager(
            config = PagingConfig(pageSize = pageSize),
            pagingSourceFactory = {
                SearchPagingSource(imagesApi = imagesApi, query = term, perPage = pageSize, maxImages = maxImages)
            }
        ).flow
    }
}